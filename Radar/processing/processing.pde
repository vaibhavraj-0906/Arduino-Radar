...
void setup() {
  fullScreen(); // Runs in full screen
  smooth();
  myPort = new Serial(this, "COM5", 9600); // starts the serial communication
  myPort.bufferUntil('.'); // reads data until '.' character
}
...
void drawText() { // draws the texts on the screen
  ...
  textSize(40);
  text("Vaibhav Raj", width - width*0.875, height - height*0.0277);
  text("Angle: " + iAngle + " °", width - width*0.48, height - height*0.0277);

  if(iDistance < 40) {
    text("Distance: " + iDistance + " cm", width - width*0.3, height - height*0.0277);
  } else {
    text("Distance: Out of Range", width - width*0.3, height - height*0.0277);
  }
